...
interface PeopleCardProps {
  people?: Person[]; // optional, defaults to []
  capacity?: number; // optional, defaults to 0
  onJoin: () => void; // callback when clicking "Join"
  onLeave: () => void; // callback when clicking "Leave"
              ...
              <div className="border rounded p-2 text-center h-100 d-flex flex-column align-items-center justify-content-center">
                {person ? (
                  <>
                    ...
                    {person.uid ===
                      (alreadyJoined &&
                        people.find((p) => p.uid === person.uid)?.uid) && (
                      <button
