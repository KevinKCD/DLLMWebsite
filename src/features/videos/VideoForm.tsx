...
const VideoForm: React.FC<VideoFormProps> = ({
  ...
  const [touched, setTouched] = useState<
    Partial<Record<keyof VideoFormValues, boolean>>
  >({});
  const [submitted, setSubmitted] = useState(false);
  const [errors, setErrors] = useState<
    Partial<Record<keyof VideoFormValues, string>>
  >({});
  ...
  const handleChange = (field: keyof VideoFormValues, value: string) => {
    setFormData((prev) => ({ ...prev, [field]: value }));
  };

  const handleBlur = (field: keyof VideoFormValues) => {
    setTouched((prev) => ({ ...prev, [field]: true }));
  };

  const validate = (): boolean => {
    const newErrors: Partial<Record<keyof VideoFormValues, string>> = {};
  ...
  const showError = (field: keyof VideoFormValues) =>
